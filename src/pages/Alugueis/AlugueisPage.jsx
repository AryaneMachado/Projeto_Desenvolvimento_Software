...
export default function AlugueisPage() {
  ...
  async function finalizarAluguel() {
    ...
    try {
      await aluguelService.finalizar(finalizando.id);
      toast.success(`Locação #${finalizando.id} finalizada. Veículo liberado!`);
  ...
  const totalReceita = alugueis.filter((a) => a.status === 'finalizado').reduce((s, a) => s + (a.valor_total || 0), 0);
  ...
  return (
    <AdminLayout>
      ...
      <ConfirmModal
        ...
        onConfirm={finalizarAluguel}
        loading={actionLoading}
        title="Finalizar Locação"
        message={`Confirma a finalização da locação #${finalizando?.id} (${finalizando?.veiculo})? O veículo será liberado automaticamente.`}
      />
                  ...
                  {filtrados.map((a) => (
                    <tr key={a.id} className="hover:bg-white/3 transition-colors group">
                      ...
                      <td className="px-5 py-4 text-right">
                        {a.status === 'ativo' && (
                          <button
                            onClick={() => { setFinalizando(a); setConfirmOpen(true); }}
                            className="opacity-0 group-hover:opacity-100 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 transition-all ml-auto"
                          >
