...
import { validarDatas, formatarMoeda, diffDias } from '../../utils/validators';
...
export default function AluguelFormPage() {
  ...
  useEffect(() => {
    Promise.all([veiculoService.listarDisponiveis(), usuarioService.listar()])
      .then(([v, u]) => { setVeiculos(v); setUsuarios(u); });
  }, []);
          ...
          <div className="space-y-5">
            ...
            <div className="p-6 rounded-2xl bg-gray-900/50 border border-white/5 space-y-4">
              <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wider flex items-center gap-2">
                <FileText size={15} className="text-indigo-400" /> Partes Envolvidas
              </h2>
              ...
              <Select id="veiculo_id" label="Veículo" value={form.veiculo_id} onChange={(e) => set('veiculo_id', e.target.value)} error={erros.veiculo_id} required>
                <option value="">Selecione o veículo</option>
                {veiculos.map((v) => (
                  <option key={v.id} value={v.id}>{v.modelo} — {v.fabricante} ({v.placa})</option>
                ))}
              </Select>
            </div>
            ...
            {/* Período */}
            <div className="p-6 rounded-2xl bg-gray-900/50 border border-white/5 space-y-4">
              <h2 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Período & Pagamento</h2>
