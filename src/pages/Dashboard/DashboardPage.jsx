...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function carregar() {
      const [veiculos, usuarios, alugueis] = await Promise.all([
        veiculoService.listar(),
        usuarioService.listar(),
        aluguelService.listar(),
      ]);
      const ativos = alugueis.filter((a) => a.status === 'ativo');
      const receita = alugueis.filter((a) => a.status === 'finalizado').reduce((s, a) => s + (a.valor_total || 0), 0);
      setStats({
        veiculos: veiculos.length,
        disponiveis: veiculos.filter((v) => v.disponivel === 1).length,
        usuarios: usuarios.length,
        alugueisAtivos: ativos.length,
        receitaTotal: receita,
      });
      setRecentes(alugueis.slice(0, 6));
      setLoading(false);
    }
    carregar();
  }, []);
        ...
        <div className="relative p-6 rounded-2xl bg-gradient-to-r from-indigo-900/40 to-purple-900/40 border border-indigo-500/20 overflow-hidden">
          ...
          <div className="relative flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1">
              <p className="text-gray-400 text-sm">Receita Total (locações finalizadas)</p>
